defmodule Tictactoe.GameCore do
  @game %{
    game_state: :playing,
    move: :x,
    winner: nil,
  ...
  @doc """
  make move, function heads to check if correct player moving, and game isn't over
  """
  def make_move(%{move: player_to_move, game_state: :playing} = game, cell, mark, player)
      when player == player_to_move do
    cell_check_result = check_cell(game, cell)
    process_move(game, cell, mark, player, cell_check_result)
  end
  ...
  defp mark_cell(game, cell, mark) do
    %{game | board: Map.put(game.board, cell, mark)}
    |> check_for_win()
  end

  defp computer_random_move(game) do
    cell_pick =
      get_open_cells(game)
      |> Enum.random()
  ...
  @doc """
  check that the cell being marked is empty
  """
  defp check_cell(game, cell) do
    case game.board[cell] do
      nil -> true
      _ -> false
    end
  end
  ...
  @doc """
  cell is open, proceed with making move
  """
  defp process_move(game, cell, mark, player, true) do
    game
    |> mark_cell(cell, mark)
  ...
  @doc """
  cell has already been marked
  """
  defp process_move(game, _cell, _mark, _player, false) do
    IO.puts("Can't mark that cell!")
    game
  end
